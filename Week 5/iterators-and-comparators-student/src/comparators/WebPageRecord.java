...
public class WebPageRecord implements Comparable<WebPageRecord> {
     ...
     * those are equal, then break ties using the lastRetrieved value's natural order.
     */
    @Override
    public int compareTo(WebPageRecord w) {
        if (URL.compareTo(w.URL) < 0) return -1;
        if (URL.compareTo(w.URL) > 0) return 1;
        if (lastRetrieved.compareTo(w.lastRetrieved) > 0) return 1;
        if (lastRetrieved.compareTo(w.lastRetrieved) < 0) return -1;
        else return 0;
    }
}
